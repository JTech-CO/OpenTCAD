...
export function MosMap({result,ko}:{result:MosResult;ko:boolean}) {
  ...
  const x=(v:number)=>60+800*v/length, y=(v:number)=>80+650*v;
  return <figure className="lab-device mos-map"><figcaption>{ko?"실제 2D 메시 결과":"Computed 2D mesh"}</figcaption>
    ...
    <svg viewBox="0 0 920 460" role="img" aria-label={ko?"실리콘과 산화막의 실제 2D 계산 지도":"Computed silicon and oxide 2D map"}>
      ...
      <path d={`M${x(0)} ${y(0)} H${x(.4)} M${x(length-.4)} ${y(0)} H${x(length)} M${x(.5)} ${y(-result.input.oxideNm*.001)} H${x(length-.5)} M${x(0)} ${y(.5)} H${x(length)}`} stroke="#ffca67" strokeWidth="5" />
      <text x={x(.15)} y="54">S</text><text x={x(length/2)} y="45" textAnchor="middle">G</text><text x={x(length-.15)} y="54">D</text>
      <text x={x(length/2)} y="438" textAnchor="middle">B · x: 0 → {length} μm</text><text x="10" y="255" transform="rotate(-90 10 255)">y: 0 → 0.5 μm</text>
...
export function MosPanel({locale,token}:{locale:"en"|"ko";token:string|null}) {
  ...
  const [pending,setPending]=useState<MosInput|null>(null),[profile,setProfile]=useState<string|null>(null);
  ...
  useEffect(()=>{if(!token)return;let closed=false;void labRequest(token,"/v1/lab/status").then(v=>{const hash=(v as {supremProfileSha256?:unknown}).supremProfileSha256;if(!closed && typeof hash==="string" && /^[a-f0-9]{64}$/u.test(hash))setProfile(hash);}).catch(()=>{});return()=>{closed=true;};},[token]);
  useEffect(()=>{
  ...
  return <section className="lab-solver" aria-label="2D MOSFET"><h2>{ko?"2D MOSFET 해석":"2D MOSFET simulation"}</h2><p>{ko?"게이트·산화막·실리콘 구조의 Poisson 및 전자·정공 연속 방정식을 풉니다. 입력 변경 후 실행하면 전류 곡선과 공간 지도를 다시 계산합니다.":"Solve Poisson and electron/hole continuity in a gate/oxide/silicon structure. Change inputs and run to recompute the current curve and spatial fields."}</p>
    ...
    <div className="lab-inputs"><label>{ko?"도핑 출처":"Doping source"}<select disabled={running} value={input.dopingMode} onChange={e=>change({...input,dopingMode:e.target.value as MosInput["dopingMode"]})}><option value="template">{ko?"기본 해석 템플릿":"Analytic template"}</option><option value="suprem" disabled={!profile}>{ko?"SUPREM 활성 도핑 가져오기":"Imported SUPREM active doping"}</option></select></label>
      {(Object.keys(mosLimits) as (keyof typeof mosLimits)[]).map(key=><label key={key}>{labels[key]}<input type="number" step="any" min={mosLimits[key][0]} max={mosLimits[key][1]} disabled={running || (input.dopingMode==="suprem" && ["acceptorsCm3","donorsCm3"].includes(key))} value={Number.isFinite(input[key])?input[key]:""} onChange={e=>change({...input,[key]:e.target.valueAsNumber})} /><small>{mosLimits[key][0]} ~ {mosLimits[key][1]}</small></label>)}
      <label>{ko?"메시 세분화":"Mesh refinement"}<select disabled={running} value={input.refinement} onChange={e=>change({...input,refinement:Number(e.target.value)})}><option value="1">1×</option><option value="2">2×</option></select></label></div>
    <p>{ko?"SUPREM 연계: 서버 시작 시 --suprem-structure 경로를 지정합니다. 활성 도핑만 고정 형상 메시로 전달하며 산화·식각 형상 전체를 가져오지는 않습니다.":"SUPREM coupling: start the service with --suprem-structure PATH. Active doping is transferred onto the template mesh; oxidation/etch geometry is not imported."}</p>
