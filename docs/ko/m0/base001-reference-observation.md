...
첫 5회 관찰은 유용한 증거지만 **BASE-001을 완료하지 않습니다**. Rootless Podman이 아닌 Docker Desktop에서 실행했고, cache 없는 재빌드의 image byte가 재현되지 않았으며, 모든 solver 실행이 exit code 0을 반환하면서도 선언된 command input 오류를 기록했습니다.
...
2. 같은 계획을 Linux amd64 rootless Podman에서 재현합니다.
3. Image build를 byte 단위로 재현 가능하게 만들고 불변 manifest와 SBOM license conclusion을 검토합니다.
4. 사용 권한이 있고 독립 검토된 fixture에서 실패하는 plot command를 교체하거나 수정합니다.
5. NMOS와 CMOS 공정 및 소자 관찰과 장애 경로 test를 추가합니다.
6. Maintainer와 반도체 수치 검토자가 명시적으로 검토한 증거만 승격합니다.
