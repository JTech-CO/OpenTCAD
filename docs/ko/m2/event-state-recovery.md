...
Dependency-free Python suite는 현재 test 77개를 포함합니다. Test 25개는 memory 및 SQLite adapter 공통 suite, outcome mapping과 partial replay, 결정론적 crash 경계 네 곳, cancellation recovery, 경쟁 claim, cleanup retry 수렴, schema 및 lock 동작, 별도 process hard exit를 검사합니다. Test는 격리된 SQLite file만 열며 runtime socket, network connection, 제품 runtime, solver는 열지 않습니다.

다음 state 마일스톤은 비활성 composition 경계 뒤에서 phase-time broker event를 연결하고 부분 operation 실패와 startup 순서 계약을 정의하는 작업입니다. Backup 및 restore, power-loss 자격 검증, distributed fencing, 제품 활성화, 제품 Docker 및 Podman adapter는 계속 gate 상태입니다.
