...
Dependency-free suite는 현재 test 39개를 포함합니다. 추가 case는 canonical output round trip, 빈 파일, 악성 member type, 압축, metadata drift, byte substitution, manifest 및 limit drift, 할당 전 case-fold collision, kind 독립 identity, 정상 cancellation, 누락 및 cross-job cancellation, object 0 cleanup, secret 또는 host path를 포함한 적대적 diagnostic 주입을 다룹니다.

## 남은 경계

제품 runtime adapter, broker service transport, runtime detection, worker integration, durable event store, restart-safe cancellation, concurrent cancellation arbitration, solver 실행, runtime socket 접근은 아직 없습니다. Lifecycle phase별 cancellation 주입과 durable crash recovery는 다음 gate 상태 작업으로 남습니다.
