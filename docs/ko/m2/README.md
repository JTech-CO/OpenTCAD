...
M2는 `gated-active` 상태입니다. 계약 검토를 위해 runtime-neutral model, stable error, fail-closed policy validator, protocol, canonical input/output archive validator, 고정 job identity, phase 지정 cancellation, redacted 공개 event, process-local cleanup coordinator, 결정론적 broker-event mapping, 재사용 가능한 state-adapter conformance suite, 재사용 가능한 native object runtime-fence conformance suite, durable-state interface, 비활성 file-backed SQLite 후보, crash/restart recovery coordinator, 비활성 mock 전용 phase-time composition, strict mock backend, 내부 mock broker orchestrator를 구현했습니다. 명시적인 비활성 composition은 mock 전용 test에서 execution과 외부 cancellation을 후보에 live wiring합니다. 외부 cancellation은 runtime query 전에 intent를 commit하고 revision CAS로 승자 하나를 정하며 restart 뒤에도 그 결정을 보존합니다. 새로운 내부 owner 시도와 단조 증가 fencing token은 검사된 store 및 broker 경계에서 execution, cancellation, recovery를 중재합니다. 공유형 process-local fence authority가 모든 bound mock operation 전후에 활성화와 verification을 수행하고 managed mock object는 정확한 fencing label을 저장하고 강제합니다. Composition은 제품 비활성 상태입니다. Docker 또는 Podman 제품 adapter, sandbox broker service transport, runtime detection, worker integration, runtime socket 접근, solver 실행은 없습니다.
...
| M2 종료 | 미충족 | Durable lease liveness, 공유형 mock authority, native object label 저장, 재사용 가능한 runtime-fence conformance는 test했지만 broker service, solver corpus, native token 강제를 갖춘 제품 runtime adapter, process 간 authority, power-loss 증거, native platform 검증이 없음 |
...
해결되지 않은 M1 진입 증거를 처리한 뒤 ADR과 threat model을 검토하고 승인해야 합니다. 향후 제품 Docker 또는 Podman adapter는 확정된 authority, label 저장, inspection, object generation 규칙, operation 전후 검사를 구현하고 공통 adapter case 5개를 변경 없이 통과한 뒤 in-flight 취소 또는 안전한 수렴에 대한 native platform 증거를 만들어야 합니다. Process 간 durable authority, store와 runtime 활성화의 원자화, backup 및 restore, power-loss 자격 검증, multi-host coordination, clock-skew policy, 제품 활성화는 이후 gate로 남습니다. 승인된 불변 engine profile과 해당 M2 진입 게이트가 생길 때까지 실제 Docker 및 Podman adapter는 차단합니다.
