...
| RUN-007 contract suite | 비활성 durable ownership 계약 test 완료 | Test 102개에 archive 공격, 20회 반복 2종, execution cancellation checkpoint 11곳, concurrent cleanup, phase-time execution 및 cancellation persistence, memory 및 SQLite adapter 공통 conformance case 7개, startup admission, 부분 write 처리, recovery crash 경계 4곳, cancellation crash 경계 5곳, ownership 경쟁 case 5개, schema v1 migration, 별도 process hard exit, reconciliation, redaction, durable-state 의미 포함 |
...
| M2 종료 | 미충족 | Mock 전용 협력형 ownership fencing은 test했지만 broker service, solver corpus, 제품 runtime adapter, runtime 강제형 fencing, owner liveness 및 lease expiry, power-loss 증거, native platform 검증이 없음 |
...
`DurableJobStateStore`는 원자적 revision CAS, event UUID 및 operation slot idempotency, 허용 transition, terminal 불변성, redacted 저장 field, 제한된 recovery scan, 정확한 ownership 검사를 고정합니다. 모든 event는 논리적 operation UUID, 새로운 owner 시도 UUID, 단조 증가 fencing token을 포함합니다. `BrokerStateMapper`와 `StateEventRecorder`는 결정론적 complete-outcome mapping 및 partial replay를 정의합니다. 공통 conformance case 7개는 non-durable memory double과 `SQLiteJobStateStore`에서 모두 실행됩니다. SQLite 후보는 append-only schema v2, WAL, `synchronous=FULL`, `BEGIN IMMEDIATE`를 사용하고 정확한 schema v1 file을 transaction으로 전진 migration하며 알 수 없는 schema를 실패 폐쇄하고 자동 retention을 수행하지 않습니다. `CrashRecoveryCoordinator`는 결정론적 recovery 경계 4곳을 검증하고 별도 Python process는 commit된 SQLite claim이 `os._exit` 뒤에도 남으며 reopen 뒤 새로운 recovery owner가 이전 owner를 fence 처리하고 수렴함을 증명합니다. `LiveStateSession`과 `DurableBrokerComposition`은 phase write를 기다리고 admission 전 startup recovery를 모두 처리하며 일반 write 실패 뒤 fail-safe cleanup을 계속하고 미완료 prefix를 recoverable 상태로 남깁니다. Execution은 token 1에서 시작하고 cancellation 및 recovery takeover는 runtime 작업 전에 token을 증가시킵니다. Ownership guard는 stale owner의 append, 감지된 takeover 뒤 검사된 runtime 작업 시작, cleanup, reconciliation, terminal state 기록을 막습니다. Stale 공개 outcome은 `operation-fenced`를 사용합니다. Broker 직접 execution과 cancellation은 기존 non-persisted 동작을 유지합니다. Service transport, 제품 활성화, runtime 강제형 fencing, owner liveness 및 lease expiry, backup 및 restore, power-loss 자격 검증은 없습니다.
...
해결되지 않은 M1 진입 증거를 처리한 뒤 ADR과 threat model을 검토하고 승인해야 합니다. 다음 ownership 범위는 durable owner liveness 및 lease-expiry policy와 향후 제품 runtime adapter의 fencing token 전달 및 강제입니다. Backup 및 restore, power-loss 자격 검증, multi-host coordination, 제품 활성화는 이후 gate로 남습니다. 승인된 불변 engine profile과 해당 M2 진입 게이트가 생길 때까지 실제 Docker 및 Podman adapter는 차단합니다.
