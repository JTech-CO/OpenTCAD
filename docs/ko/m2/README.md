...
M2는 `gated-active` 상태입니다. 계약 검토를 위해 runtime-neutral model, stable error, fail-closed policy validator, protocol, canonical input archive validator, strict mock backend, 내부 mock broker orchestrator를 구현했습니다. Docker 또는 Podman 제품 adapter, sandbox broker service transport, runtime detection, worker integration, runtime socket 접근은 없습니다.
...
| RUN-007 contract suite | Mock broker test 완료 | Test 29개에 archive 공격, 20회 반복 2종, reconciliation, cleanup negative control 포함 |
...
| BRK-004 archive defense | 기반 test 완료 | Traversal, link, device, 압축, metadata, collision, bomb, drift를 fail closed 처리 |
...
Mock backend는 process를 실행하지 않고 socket도 소유하지 않습니다. 내부 broker library는 memory에서 정확한 비압축 USTAR stream을 검증하고 staging 경계에는 `ValidatedInputArchive`만 허용하며 container 다음 volume 순서로 제거한 뒤 cleanup 성공을 보고하기 전에 managed object를 다시 조회합니다. Service transport는 노출하지 않으며 Docker와 Podman 구현이 허용되기 전에 lifecycle, archive, cleanup, error 의미를 재사용 가능하게 고정하는 용도입니다.
...
해결되지 않은 M1 진입 증거를 처리한 뒤 ADR과 threat model을 검토하고 승인해야 합니다. 다음 gate 상태 구현 범위는 mock broker의 output archive 검증, cancellation identity, structured event redaction 장애 test입니다. 승인된 불변 engine profile과 해당 M2 진입 게이트가 생길 때까지 실제 Docker 및 Podman adapter는 차단합니다.
