...
M2는 `gated-active` 상태입니다. 계약 검토를 위해 runtime-neutral model, stable error, fail-closed policy validator, protocol, canonical input/output archive validator, 고정 job identity, phase 지정 cancellation, redacted 공개 event, process-local cleanup coordinator, 결정론적 broker-event mapping, 재사용 가능한 state-adapter conformance suite, durable-state interface, 비활성 file-backed SQLite 후보, crash/restart recovery coordinator, strict mock backend, 내부 mock broker orchestrator를 구현했습니다. SQLite 후보는 제품에 활성화하거나 live wiring하지 않았습니다. Docker 또는 Podman 제품 adapter, sandbox broker service transport, runtime detection, worker integration, runtime socket 접근, solver 실행은 없습니다.
...
| RUN-007 contract suite | SQLite 후보 restart 계약 test 완료 | Test 77개에 archive 공격, 20회 반복 2종, cancellation checkpoint 11곳, concurrent cleanup, event mapping, memory 및 SQLite adapter conformance, 주입 crash 경계 4곳, 별도 process hard exit, reconciliation, redaction, durable-state 의미 포함 |
...
| BRK-006 recovery 및 reconciliation | SQLite 후보 hard-exit recovery test 완료 | 공유 job lease로 mock cleanup 직렬화, 공통 adapter test로 memory 및 SQLite CAS 검증, commit된 SQLite claim이 별도 process hard exit 뒤에도 남고 정확한 job reconciliation으로 수렴 |
...
| M2 종료 | 미충족 | SQLite adapter는 비활성 후보이며 live phase-time wiring, broker service, solver corpus, 제품 runtime adapter, power-loss 증거, distributed ownership, native platform 검증이 없음 |
...
해결되지 않은 M1 진입 증거를 처리한 뒤 ADR과 threat model을 검토하고 승인해야 합니다. 다음 state 범위에서는 검토된 SQLite 후보를 비활성 composition 경계 뒤에서 phase-time broker event와 연결하고 runtime 권한을 넓히지 않은 채 부분 operation 실패와 startup 순서를 정의해야 합니다. Backup 및 restore, power-loss 자격 검증, distributed fencing, 제품 활성화는 이후 gate로 남습니다. 승인된 불변 engine profile과 해당 M2 진입 게이트가 생길 때까지 실제 Docker 및 Podman adapter는 차단합니다.
