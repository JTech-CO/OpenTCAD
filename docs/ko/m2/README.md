...
| RUN-007 contract suite | 비활성 durable authority 및 native object fencing conformance test 완료 | Test 154개에 archive 공격, 20회 반복 2종, execution cancellation checkpoint 11곳, concurrent cleanup, phase-time execution 및 cancellation persistence, memory 및 SQLite state adapter 공통 conformance case 10개, startup admission, 부분 write 처리, recovery crash 경계 4곳, cancellation crash 경계 5곳, ownership 경쟁 case 5개, state schema v1 및 v2 migration, owner lease case 4개, runtime object fence 공통 case 5개, native object 집중 case 3개, authority 공통 case 4개, SQLite authority case 4개, store와 runtime 활성화 case 4개, 별도 process hard-exit case 8개, 조정된 offline snapshot case 7개, backup-control case 7개, 인증 backup-service case 7개, reconciliation, redaction, durable-state 의미 포함 |
...
| M2 종료 | 미충족 | Durable lease liveness, local process 간 authority 후보, 활성화 gap 복구, native object label, 조정된 offline pair restore, 재사용 가능한 conformance를 test했지만 broker service, solver corpus, native token 강제를 갖춘 제품 runtime adapter, store와 runtime 활성화 원자성, power-loss 증거, native platform 검증이 없음 |
...
- [조정된 SQLite offline snapshot 및 restore](sqlite-offline-snapshot-restore.md)
- [인증된 backup control 및 durability](authenticated-backup-control.md)
...
- [조정된 SQLite offline snapshot manager](../../../backend/app/broker/sqlite_snapshot.py)
- [Durable backup control store](../../../backend/app/broker/backup_control.py)
- [인증된 backup service 및 scheduler](../../../backend/app/broker/backup_service.py)
...
- [조정된 offline snapshot 및 hard-exit test](../../../backend/tests/broker/test_sqlite_snapshot.py)
- [Durable backup control test](../../../backend/tests/broker/test_backup_control.py)
- [인증 backup, schedule, hard-exit test](../../../backend/tests/broker/test_backup_service.py)
...
해결되지 않은 M1 진입 증거를 처리한 뒤 ADR과 threat model을 검토하고 승인해야 합니다. 향후 제품 Docker 또는 Podman adapter는 composition이 사용하는 동일 authority를 노출하고 정확한 object label을 저장 및 inspection하며 native operation 전후에 generation 규칙을 강제해야 합니다. 또한 object case 5개와 authority case 4개를 변경 없이 통과하고 in-flight 취소 또는 안전한 수렴에 대한 native platform 증거를 만들어야 합니다. Broker admission 및 service transport 연결, OS credential storage, scheduler 자동 wake-up, 보호된 control store rollback 저항성, 원자적이거나 별도로 공식 조정된 store와 runtime 활성화, 실제 power-loss 자격 검증, multi-host coordination, clock-skew policy, 제품 활성화는 이후 gate로 남습니다. 승인된 불변 engine profile과 해당 M2 진입 게이트가 생길 때까지 실제 Docker 및 Podman adapter는 차단합니다.
