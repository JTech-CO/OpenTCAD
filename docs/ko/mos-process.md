...
기본 **SUPREM 활성 도핑 가져오기** 모드는 도핑만 전달합니다. 별도의
[원본 메시 모드](process-mesh.md)는 실리콘·산화막 삼각형과 명시적 전극 경계를
유지합니다. 다음 보간 규칙은 도핑-only 모드에 대한 설명입니다.
...
플랫폼별 SUPREM 자격 검증, 지원 실리콘·산화막을 넘어서는 범용 다중 재료 전달, 보정된 MOS 수치 기준,
