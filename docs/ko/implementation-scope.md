...
이 문서는 읽기 전용 동작 참고본 `ypooh2042/tcad-webapp@13bce4a`와 `JTech-CO/OpenTCAD@4a7cae9`에 병합된 OpenTCAD 기반을 비교합니다. 참고본의 README, 코드맵, 프런트엔드와 백엔드 패키지, 컨테이너 정의, 배포 구조를 확인했습니다. 업스트림 애플리케이션 소스는 OpenTCAD에 복사하지 않았습니다.
...
|---|---|---|---|
| 공개 정적 접근 | 별도의 안전한 정적 제품 없음 | GitHub Pages에 완료 | 로드맵 전체에서 유지 |
...
| 단계 | 구현 가능한 결과 | 필수 종료 증거 |
|---|---|---|
| 기반, 완료 | 한·영 정적 제품, CI, Pages, 아키텍처·라이선스 경계 | 정적 빌드에서 입력 실행 불가 |
| M0 | 의존성·출처 목록, 배포 결정, Linux 동작 기준선 | 문서화된 라이선스 결정과 재현 가능한 기준선 |
| M1 | 골든 덱, 파서, 수치 comparator, 고정 이미지 | 반복 가능한 topology, 공정 지표, I–V 증거 |
| M2 | 런타임 계약, 브로커, Docker·Podman adapter, 잡 볼륨 | fail-closed 샌드박스 테스트와 정리 증거 |
| M3 | loopback 로컬 스택, launcher, doctor, 프로젝트, 백업·업그레이드 골격 | 런타임 소켓 노출 없는 단일 명령 로컬 alpha |
| M4부터 M5 | Windows·macOS 패키징과 architecture 정책 | 지원 호스트의 반복 E2E와 수치 증거 |
| M6부터 M8 | correctness 개선, 이식성, 진단, 릴리스 운영 | 중요 결함 0건과 라이선스·rollback·자격 게이트 충족 |

실용적인 로컬 alpha는 M0부터 M3까지 완료하면 가능합니다. 신뢰할 수 있는 1.0에는 이후 수치·보안·데이터 안전·플랫폼 게이트가 필요하며, 프로세스 종료만으로 성공을 판정하지 않습니다.
