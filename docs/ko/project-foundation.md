...
- 기존 기획 내용과 제공된 UI 참고 이미지는 추적 가능해야 하며, 이후 문장부호만 정규화할 때는 기록과 해시를 갱신합니다.
...
- `npm run check:ko-copy`: 한국어 문서 13개와 UI 번역에서 `U+2014` em dash 0건
- `npm run lint`: 경고 없이 통과
- `npm run test`: 2개 파일, 9개 테스트 통과
...
- 문장부호를 정규화한 기획 묶음: 기록된 SHA-256 해시 8개 모두 일치

## 롤백

기반 작업은 PR #1의 병합 커밋 `4a7cae9`로 반영되었습니다. 해당 병합을 되돌리면 저장소가 초기 README 상태로 돌아가며 데이터베이스, 런타임 객체, 솔버 산출물 또는 사용자 프로젝트에는 영향을 주지 않습니다.
