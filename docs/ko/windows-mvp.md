...
Windows 11 x64를 대상으로 하는 별도의 소스 배포 릴리스 트랙입니다.
...
같은 작업 ID에 다른 입력을 보내면 충돌로 거부합니다. 웹의 '이력·비교'에서
'이력 새로고침'을 누르면 PN·2D 결과, 출처와 비교를 확인하고 내보낼 수 있습니다.
조회는 입력을 덮어쓰거나 새 계산을 실행하지 않습니다. 새로고침하면 메모리의
접근 토큰이 사라지므로 개인용 서비스 주소로 다시 접속하세요. 브라우저 영속
저장소에 토큰을 보관하지 않습니다. 서버 중지 후 CLI로 조회하려면 다음을 사용합니다.
...
깨끗한 체크아웃에서 `npm run accept:windows -- --output C:\absolute\new-evidence`를
실행하면 전체 검사, doctor, 실제 솔버 시험을 순서대로 수집합니다. 커밋, 호스트,
로그와 해시를 남기며 소스가 바뀌면 실패합니다. 자동 관측일 뿐 자체 릴리스 승인이
아닙니다. `.experimental-results`의 새 하위 폴더처럼 Git에서 제외한 증거 폴더를
사용하세요. AI 클라이언트 연결은 [로컬 MCP 안내](mcp.md)를 참고하세요.

[로컬 관측 기록](../../validation/experimental/windows-mvp-observation.json)에는 네이티브
테스트 35개 통과와 함께, 앞선 PN 반복 결과 해시 불일치 1건도 보존했습니다.
이후 사용자는 Codex 서브에이전트 동시 작업 중 외부 로그 손상이 원인이었다고
설명했습니다. [사건 종결 기록](../../validation/experimental/pn-incident-disposition-20260912.json)은
이 사용자 보고를 근거로 과거 차단을 해제하며, 원인을 독립 재현했거나 솔버를
수정했다고 주장하지 않습니다. 원본 관측과 정확한 일치 검사는 유지하고
새로운 불일치는 별도로 조사합니다.
이후 전용 호스트 시험에서 같은 해시 쌍이 재현되었고 장비 메타데이터만 다른 것을
확인했습니다. [새 원인 증거와 수정](pn-repeatability.md)에 따라 WMI 대체 경로 대신
Windows 네이티브 조회를 사용합니다.
[PN 조사 안내](pn-repeatability.md)의 도구로 원본 반복 결과와 항목별 차이를
보존할 수 있으며 승인 검사는 완화하지 않습니다.

물리적 전원 차단 내구성과 독립 기관 자격 검증은 이 릴리스에서 요구하거나
보장하지 않습니다. macOS·Linux 제품 자격, 자동 백업, OS 보호 롤백 방지,
솔버 포함 배포의 재배포 검토와 외부 보정 정확도는 미검증·유보 상태입니다.
외부 corpus 승인 없이도 수치 회귀와 결과 재실행 검증은 필수 조건으로 유지합니다.
