...
On 2026-08-24, the BASE-001 observer also read the public 1D boron example deck and compared the generated structure only by hash with the frozen test-fixture Git blob. The observer built and ran the frozen external tree locally, inspected externally visible logs, and recorded normalized facts. Raw source, patches, binaries, images, logs, structures, plots, and the SBOM remain outside OpenTCAD. A second rootless Podman observation used the same frozen input and recorded only profile checks, hashes, counts, log findings, and local image identities; no Podman image archive or SBOM entered OpenTCAD. A controlled follow-up verified the external Containerfile by hash, generated a digest- and snapshot-pinned recipe outside OpenTCAD, rebuilt the local image twice, and generated a network-disabled SBOM from an external OCI archive. The generated recipe, build logs, image archive, and raw SBOM remain external; only normalized identities and counts enter OpenTCAD. The observation harness was independently written from OpenTCAD evidence requirements and does not reproduce reference application control flow or organization.
