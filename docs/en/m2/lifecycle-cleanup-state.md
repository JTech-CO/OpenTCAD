...
`InMemoryJobStateStore` remains a non-durable contract double. `SQLiteJobStateStore` is an inactive file-backed candidate that runs the same conformance suite and adds schema v1, migration refusal, append-only retention, lock redaction, and separate-process hard-exit tests. An explicit mock-only composition can persist execution phase events to either store after startup recovery; direct broker execution and external cancellation remain non-persisted. PostgreSQL, automatic compaction, backup and restore, distributed leases, power-loss qualification, and product activation are not implemented or claimed.

## Verification and remaining gates

The Python 3.12 to 3.14 suite contains 86 tests. The cases exercise all eleven cancellation checkpoints, wrong-identity and non-boolean signals, four concurrent reconciliations sharing one coordinator, already-absent runtime responses, cancel/reconcile convergence, monotonic revisions, identical replay, conflicting event reuse, concurrent CAS, transition rejection, terminal immutability, recovery pagination, persisted-shape redaction, startup admission, phase-time ordering, and partial-write cleanup.

Product Docker and Podman adapters, runtime sockets, broker service transport, external cancellation persistence, distributed cleanup ownership, power-loss durability, restart-safe cancellation transport, and solver execution remain blocked or pending under the existing gates. See [event mapping, adapter conformance, and restart recovery](event-state-recovery.md) and the [SQLite durable-state candidate](sqlite-durable-state.md) for the tested contracts.
