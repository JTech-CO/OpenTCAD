...
This authority adapter is local-host evidence only. The separate [coordinated offline snapshot contract](sqlite-offline-snapshot-restore.md) now covers candidate pair backup and fresh-target restore, but the authority has no integrated retention, product backup service, authenticated restore, durable rollback floor, corruption repair, multi-host consensus, bounded clock-skew proof, or filesystem power-loss qualification. It does not revoke an already submitted native call. A product Docker or Podman adapter must persist and enforce the same generation on native objects, use the same authority instance exposed to the composition, pass both reusable conformance suites unchanged, and provide native crash, power-loss, and safe in-flight convergence evidence before activation.
