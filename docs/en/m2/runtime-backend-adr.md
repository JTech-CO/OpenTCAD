...
- Canonical input/output archive validation, fixed job identity, phase-addressable cancellation, process-local cleanup serialization, public/internal diagnostic separation, deterministic outcome and mock-only phase-time mapping, the common adapter conformance suite, the durable-state interface, an inactive SQLite candidate, startup recovery admission, partial-write cleanup, durable external-cancellation CAS arbitration, five cancellation restart seams, and separate-process hard-exit recovery are implemented in the broker foundation; product runtime transfer, durable ownership fencing, external cancellation transport, backup, and power-loss qualification remain separate gated responsibilities.
