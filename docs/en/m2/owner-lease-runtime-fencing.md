...
The mock adapter receives a RuntimeFenceAuthority. The shared process-local reference authority retains the highest activated generation for each job across adapter instances. Every bound call activates immediately before adapter work and verifies immediately after return. Managed mock volumes and containers persist the exact job, owner, and token labels; every operation reconstructs and checks the observed context. Normal lifecycle work requires an exact generation, while a higher takeover may only query, kill, or clean predecessor objects. Lower tokens, same-token owner ambiguity, malformed labels, cross-job contexts, and mismatched handles fail closed.
...
The common memory and SQLite state-adapter conformance suite has ten cases. The reusable runtime-fence suite adds five common adapter cases for label persistence, shared-authority fencing, owner ambiguity, takeover restrictions, and cross-job rejection. Three focused cases cover malformed labels, verification before activation, and post-mutation stale-result suppression followed by current-owner cleanup convergence. Dedicated lease and recovery tests continue to cover heartbeat renewal, cancellation of an expired in-flight awaitable, schema v2 migration, and hard-exit takeover. The dependency-free backend suite contains 121 tests.

## Deliberate limits

This is durable liveness plus native-object contract enforcement for the inactive local candidate and strict mock adapter, not proof for a product OCI adapter or distributed system. Store commit and runtime activation are not one atomic operation. The authority is process-local and mock object metadata is not Docker or Podman metadata. A submitted native operation may complete a mutation after takeover; the post-operation check suppresses its stale result and current-owner cleanup can converge, but this does not prove native revocation or rollback.

The system clock can move and independently configured hosts can disagree. There is no database service, cross-process authority, clock-skew bound, multi-host lease, quorum, product adapter qualification, power-loss qualification, or backup and restore proof. A future Docker or Podman adapter must implement the same label, authority, inspection, and operation rules, pass the common suite unchanged, and produce native-platform fault evidence before this gate can be promoted.
