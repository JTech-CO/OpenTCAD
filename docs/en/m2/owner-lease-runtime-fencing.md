...
The common memory and SQLite state-adapter conformance suite has ten cases. The reusable runtime-fence suite adds five common adapter cases for label persistence, shared-authority fencing, owner ambiguity, takeover restrictions, and cross-job rejection. Three focused cases cover malformed labels, verification before activation, and post-mutation stale-result suppression followed by current-owner cleanup convergence. Dedicated lease and recovery tests continue to cover heartbeat renewal, cancellation of an expired in-flight awaitable, schema v2 migration, and hard-exit takeover. The dependency-free backend suite contains 140 tests.
