...
| State confusion and unsafe retry | Map contiguous broker events to deterministic event and operation slots; persist with atomic revision CAS, legal transitions, terminal immutability, and bounded recovery scans | Mapping, partial replay, common conformance on memory and SQLite adapters, lock redaction, and restart recovery tests exist; live broker wiring, retention compaction, backup, and distributed fencing remain pending |
