...
| Stale state or orphan reuse | Reject existing labelled objects, use exact generated identities, serialize cleanup by job, increment durable owner generations before cancellation or recovery, fence stale owners at checked mutations, cleanup container before volume, accept already-absent convergence, and query zero orphans | Process-local leases, five ownership competitions, concurrent guarded mock reconciliation, four recovery restart boundaries, five cancellation restart boundaries, and SQLite hard-exit takeover proof exist; runtime-enforced fencing, owner liveness, runtime-backed reconciliation, and power-loss recovery remain pending |
| State confusion and unsafe retry | Map contiguous broker events to deterministic event and operation slots; persist owner UUID and fencing token with atomic revision CAS, legal takeover states, terminal immutability, exact ownership verification, and bounded recovery scans | Owner-aware complete-outcome and phase-time mapping, partial replay, seven-case conformance on memory and SQLite adapters, startup admission, intent-before-runtime cancellation takeover, schema-v1 migration, lock redaction, and restart recovery tests exist; retention compaction, backup, runtime-enforced fencing, lease expiry, and multi-host coordination remain pending |
