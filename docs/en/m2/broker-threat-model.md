...
| State confusion and unsafe retry | Map contiguous broker events to deterministic event and operation slots; persist owner UUID, fencing token, lease duration, and current expiry with atomic revision CAS, revision-neutral renewal, legal takeover states, terminal immutability, exact live ownership verification, and bounded recovery scans | Owner-aware complete-outcome and phase-time mapping, partial replay, ten-case conformance on memory and SQLite adapters, live-owner recovery refusal, cancellation preemption, schema-v1 and schema-v2 migration, lock redaction, restart recovery, and a product-disabled coordinated offline pair snapshot exist; retention compaction, product backup scheduling, authenticated metadata, durable rollback-floor storage, product-native fencing, bounded clock skew, and multi-host coordination remain pending |
| Snapshot substitution, rollback, or partial restore | Require explicit offline assertion, fixed files, canonical metadata, hashes, exact schemas, state-authority generation coherence, exact source identity, a trusted minimum sequence, and fresh-directory publication | Candidate rejects torn, extra, tampered, mixed, identity-mismatched, below-floor, and existing-target cases; authentication, durable floor storage, running-service coordination, and power-loss proof remain pending |
