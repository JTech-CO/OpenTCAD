...
M2 is `gated-active`. The runtime-neutral model, stable errors, fail-closed policy validator, protocol, canonical input/output-archive validators, fixed job identity, phase-addressable cancellation, redacted public events, process-local cleanup coordinator, deterministic broker-event mapping, reusable state-adapter conformance suite, reusable native-object and authority conformance suites, durable-state interface, inactive file-backed SQLite state and runtime-fence candidates, crash/restart recovery coordinator, inactive mock-only phase-time composition, strict mock backend, and internal mock broker orchestrator are implemented for contract review. The explicit inactive composition live-wires execution, external cancellation, recovery, and the durable store-to-runtime activation bridge for mock-only tests. External cancellation commits intent before runtime query, uses revision CAS for one winner, and preserves that decision across restart. Fresh internal owner attempts and monotonically increasing fencing tokens arbitrate execution, cancellation, and recovery at checked store, authority, and broker boundaries. A process-local reference authority and a cross-process SQLite candidate activate and verify bound mock operations, while managed mock objects persist and enforce exact fencing labels. A coordinated offline snapshot candidate validates and publishes the state-authority pair and restores it only to a fresh directory. A separate inactive application-service candidate adds durable maintenance admission, HMAC-authenticated export/import, a monotonic external restore floor, persisted interval schedules, and platform publication barriers. The composition and all SQLite candidates remain product-disabled. No Docker or Podman product adapter, sandbox broker service transport, runtime detection, worker integration, runtime socket access, or solver execution exists.
...
| RUN-007 contract suite | Inactive durable authority and native-object fencing conformance tested | 154 tests include archive attacks, two 20-case loops, eleven execution cancellation checkpoints, concurrent cleanup, phase-time execution and cancellation persistence, ten-case memory and SQLite state-adapter conformance, startup admission, partial-write handling, four recovery crash boundaries, five cancellation crash boundaries, five ownership competition cases, schema-v1 and schema-v2 state migration, four owner-lease cases, five common runtime-object fence cases, three focused native-object cases, four common authority cases, four SQLite authority cases, four store-to-runtime activation cases, eight separate-process hard-exit cases, seven coordinated offline snapshot cases, seven backup-control cases, and seven authenticated backup-service cases, reconciliation, redaction, and durable-state semantics |
...
| M2 exit | Not met | Durable lease liveness, a local cross-process authority candidate, activation-gap recovery, native-object labels, coordinated offline pair restore, and reusable conformance are tested, but broker service, solver corpus, product runtime adapters with native token enforcement, atomic store-to-runtime activation, power-loss proof, and native platform qualification are absent |
...
`DurableJobStateStore` fixes atomic revision CAS, event UUID and operation-slot idempotency, legal transitions, terminal immutability, redacted persisted fields, bounded recovery scanning, and exact ownership verification. Every event carries a logical operation UUID, fresh owner-attempt UUID, and monotonically increasing fencing token. `BrokerStateMapper` and `StateEventRecorder` define deterministic complete-outcome mapping and partial replay. The ten-case common conformance suite runs against both the non-durable memory double and `SQLiteJobStateStore`. Every committed event carries a bounded lease duration; renewal extends the mutable current-lease row without changing the job revision or append-only event history. The state candidate uses schema v3, WAL, `synchronous=FULL`, and `BEGIN IMMEDIATE`; it migrates exact schema v1 files through v2 and exact schema v2 files to v3 transactionally, fails closed on unknown schemas, and performs no automatic retention. `CrashRecoveryCoordinator` verifies four deterministic recovery boundaries. `LiveStateSession` and `DurableBrokerComposition` await phase writes, drain startup recovery before admission, continue fail-safe cleanup after ordinary write failure, and leave incomplete prefixes recoverable. Execution begins at token 1; cancellation and expired-owner recovery takeovers increment the token before runtime work, while recovery reports `owner-active` for a live lease. Ownership guards renew before, during, and after guarded awaits, cancel the Python awaitable if ownership is lost, and prevent an expired or stale owner from appending or writing a terminal state. Every job lifecycle call is made through a `RuntimeFencingContext`; managed mock volumes and containers persist the exact three fencing labels and revalidate them on every operation. `SQLiteRuntimeFenceAuthority` adds a dedicated schema-v1 WAL/FULL authority that serializes the highest generation across local processes. `DurableRuntimeFenceActivator` verifies exact store ownership, activates the authority, then rechecks both before runtime contact. A hard-exit state-ahead-of-authority gap recovers by claiming and activating the next token before reconciliation. This bridge is fail-closed but the separate database commits are not atomic. The strict mock runtime still rejects lower tokens, same-token owner ambiguity, malformed labels, cross-job contexts, and mismatched handles. Post-operation authority verification suppresses stale success after an in-flight mock mutation and the current owner converges cleanup to zero objects. `SQLiteOfflineSnapshotManager` now locks state then authority, creates two standalone SQLite backup payloads, validates canonical metadata, hashes, schemas, and generation coherence, and publishes only a complete manifest-last directory. Restore requires exact snapshot and source-instance IDs plus a minimum sequence, never overwrites an existing target, republishes the pair as one fresh directory, and resumes generation-safe recovery. `SQLiteBackupControlStore` persists fenced maintenance ownership, leased operation admissions, snapshot sequence reservations, restore floors, and interval schedule claims outside the exported pair. `AuthenticatedSQLiteBackupService` drains registered admissions, signs canonical outer records with HMAC-SHA256, advances the floor before fresh-target import, and uses durable platform publication barriers. `ScheduledSQLiteBackupRunner` executes one due occurrence without automatic retention deletion. Direct broker execution and cancellation retain their prior non-persisted behavior and are not yet automatically registered with the maintenance coordinator. Service transport, product activation, product Docker or Podman token persistence and enforcement, native in-flight revocation, broker-integrated backup admission, authenticated transport, and protected control-store rollback resistance, multi-host coordination, clock-skew qualification, and power-loss qualification remain absent.
...
- [Durable runtime fence authority and activation recovery](durable-runtime-fence-authority.md)
- [Coordinated SQLite offline snapshot and restore](sqlite-offline-snapshot-restore.md)
- [Authenticated backup control and durability](authenticated-backup-control.md)
- [`RuntimeBackend` protocol](../../../backend/app/runtime/protocol.py)
- [Runtime fence authority](../../../backend/app/runtime/fence_authority.py)
- [SQLite runtime fence authority candidate](../../../backend/app/runtime/sqlite_fence_authority.py)
- [Durable store-to-runtime activation bridge](../../../backend/app/broker/runtime_fence_activation.py)
- [Coordinated SQLite offline snapshot manager](../../../backend/app/broker/sqlite_snapshot.py)
- [Durable backup control store](../../../backend/app/broker/backup_control.py)
- [Authenticated backup service and scheduler](../../../backend/app/broker/backup_service.py)
- [Cross-platform durable publication](../../../backend/app/broker/durability.py)
...
- [Runtime authority common conformance suite](../../../backend/tests/runtime/fence_authority_conformance.py)
- [SQLite runtime authority and hard-exit tests](../../../backend/tests/runtime/test_sqlite_fence_authority.py)
- [Durable activation and restart-gap tests](../../../backend/tests/broker/test_durable_fence_activation.py)
- [Coordinated offline snapshot and hard-exit tests](../../../backend/tests/broker/test_sqlite_snapshot.py)
- [Durable backup control tests](../../../backend/tests/broker/test_backup_control.py)
- [Authenticated backup, schedule, and hard-exit tests](../../../backend/tests/broker/test_backup_service.py)
...
Review and approve the ADR and threat model only after the unresolved M1 entry evidence is addressed. A future product Docker or Podman adapter must expose the same authority used by composition, persist and inspect exact object labels, enforce generation rules before and after native operations, pass the five object cases and four authority cases unchanged, and produce native-platform evidence for in-flight revocation or safe convergence. Broker admission and service-transport wiring, OS credential storage, automatic scheduler wake-up, protected control-store rollback resistance, atomic or otherwise formally coordinated store-to-runtime activation, real power-loss qualification, multi-host coordination, clock-skew policy, and product activation remain later gates. Real Docker and Podman adapters remain blocked until an approved immutable engine profile and the applicable M2 entry gates exist.
