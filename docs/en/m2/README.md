...
| M2 exit | Not met | Durable lease liveness and strict mock-runtime fencing are tested, but broker service, solver corpus, product runtime adapters with native token enforcement, power-loss proof, and native platform qualification are absent |
...
Review and approve the ADR and threat model only after the unresolved M1 entry evidence is addressed. The next ownership boundary is product Docker and Podman persistence and native enforcement of the established runtime fencing context, with explicit evidence for in-flight revocation or convergence. Backup and restore, power-loss qualification, multi-host coordination, clock-skew policy, and product activation remain later gates. Real Docker and Podman adapters remain blocked until an approved immutable engine profile and the applicable M2 entry gates exist.
