...
| RUN-007 contract suite | Inactive live-state startup contract tested | Eighty-six tests include archive attacks, two 20-case loops, eleven cancellation checkpoints, concurrent cleanup, phase-time event persistence, memory and SQLite adapter conformance, startup admission, partial-write handling, four injected crash boundaries, a separate-process hard exit, reconciliation, redaction, and durable-state semantics |
...
| BRK-006 recovery and reconciliation | Candidate startup recovery gate tested | Shared job leases serialize mock cleanup; startup drains recoverable pages before admission; common adapter tests verify memory and SQLite CAS; a committed SQLite claim survives a separate-process hard exit and converges through exact-job reconciliation |
...
| BRK-008 structured events/redaction | Phase-time mapping and redaction tested | Deterministic complete-outcome and live execution mapping persist only normalized fields; public and persisted shapes omit raw detail |
| M2 exit | Not met | Mock-only phase-time execution wiring is tested, but external cancellation persistence, broker service, solver corpus, product runtime adapters, power-loss proof, distributed ownership, and native platform qualification are absent |
...
Review and approve the ADR and threat model only after the unresolved M1 entry evidence is addressed. The next state slice is durable external cancellation and restart-safe cancellation arbitration without widening runtime authority. Backup and restore, power-loss qualification, distributed fencing, and product activation remain later gates. Real Docker and Podman adapters remain blocked until an approved immutable engine profile and the applicable M2 entry gates exist.
