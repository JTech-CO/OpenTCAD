...
The companion [output, cancellation, and redaction foundation](output-cancellation-redaction.md) validates artifact bytes and fixes cancellation identity and public diagnostics. The [SQLite durable-state candidate](sqlite-durable-state.md) proves file-backed state and process hard-exit recovery without enabling the product. A separate inactive mock-only composition persists execution and external-cancellation phase events after startup recovery, with the cancellation contract detailed in [durable external cancellation arbitration](durable-cancellation-arbitration.md). There is still no Docker or Podman product adapter, broker service transport, runtime detection, worker integration, or external cancellation transport. Real runtime work remains blocked by the M2 entry conditions and an approved immutable engine profile.
