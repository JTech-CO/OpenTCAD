...
- 제품 결정, 수치, 코드 예시, 표의 의미는 바꾸지 않았으며 `PLANNING_BUNDLE_SHA256.txt`는 정규화된 현재 파일을 기준으로 다시 생성한다.
...
| `03_MILESTONE_ROADMAP_KR.md` | 단계별 일정, 선행조건, 산출물, 진입·종료 기준, 릴리스 계획 |
...
5. **라이선스가 첫 번째 Go/No-Go 항목이다.** 현재 저장소 루트에는 GitHub가 인식하는 라이선스가 없고, 포함된 SUPREM-IV.GS 상류 라이선스는 일반적인 MIT가 아니며 상업적 재판매·임대·라이선스 등 상업 거래를 제한한다. 앱 코드와 번들된 시뮬레이터의 배포 정책을 분리해 확정해야 한다.

## 문서 사용 순서

1. `01_PRODUCT_TECHNICAL_PLAN_KR.md`의 ADR과 Go/No-Go 결정을 확정한다.
2. 확정된 결정을 `02_CODEX_HARNESS_KR.md` 상단의 프로젝트 변수에 반영한다.
3. `03_MILESTONE_ROADMAP_KR.md`의 M0부터 작업을 시작한다.
