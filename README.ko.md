...
이 소스는 안전한 정적 작업공간을 빌드합니다. 공개 미리보기는 배포된 `main` 리비전을 따르므로 개발 브랜치의 변경이 아직 반영되지 않았을 수 있습니다. 작업공간은 다음 항목을 제공합니다.
...
이 빌드는 제출한 입력, 컨테이너, SUPREM-IV.GS, Gmsh 또는 DEVSIM을 **실행하지 않습니다**. 로컬 제품 연결 계층은 구현했지만 활성화는 실패 폐쇄 상태입니다. Native runtime, 3개 platform, 실제 전원 차단, solver license, 불변 image, SBOM, 수치 corpus 승인이 끝나지 않아 커밋된 M3 매니페스트는 런타임 권한을 부여하지 않습니다. 이 저장소는 제3자 solver source나 binary를 배포하지 않습니다.
