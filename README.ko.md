...
OpenTCAD은 일반 컴퓨터에서 사용하는 오픈 소스 한영 반도체 공정·소자 시뮬레이션 작업공간입니다. 즉시 해석식 계산, 선택형 로컬 솔버 실행과 참조 UI를 제공합니다. 재현 가능한 연구·자동화 플랫폼과 전용 MCP 연동을 개발 방향으로 삼으며, 계획한 기능이 모두 구현된 것은 아닙니다.
